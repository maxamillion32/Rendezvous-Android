...
public class AddTheatre extends ActionBarActivity implements View.OnClickListener
{
    ...
    public void onClick(View v)
    {
        ...
        if(v == done)
        {
            ...
            if (director.getText().toString().equals("") || s_event_name.equals("") || s_address.equals("") || s_quota == -1 ||
                    !isTimeSet || !isDateSet) {
            ...
            if( check1 && check2 && check3) {
                ...
                User user = new User(ParseUser.getCurrentUser().getObjectId(), this);
                ParseObject theatre = new ParseObject("Events");

                theatre.put( "rating", 0);
                theatre.put("name", s_event_name);
                theatre.put("organizerID", ParseUser.getCurrentUser().getObjectId());
                theatre.put("comments", new ArrayList<String>());
                theatre.put("city", sCity.getSelectedItem());
                theatre.put("date", year * 10000 + month * 100 + day);
                theatre.put("address", s_address);
                theatre.put("category", "Theatre");
                theatre.put("subscribers", new ArrayList<String>());
                theatre.put("key", (int) (Math.random() * 1000000));
                theatre.put("isMapClicked", flag2);
                theatre.put("latitude", lat);
                theatre.put("longitude", lon);

                Bitmap btm;
                if (eventImage == null) {
                    btm = BitmapFactory.decodeResource(getResources(), R.drawable.theatre);
                } else {
                ...
                theatre.put("eventImage", file);

                theatre.put("videoLink", s_video);
                theatre.put("ticketLink", s_ticket);
                theatre.put("description", s_description);
                theatre.put("quota", s_quota);
                theatre.put("director", s_director );
                theatre.put("time", hour * 100 + min);
                theatre.put("organizerName", user.getUsername());
                ...
                if( ParseUser.getCurrentUser().getParseFile("profilePhoto") != null ) {
                    ...
                    theatre.put("organizerPhoto", file2);
                }
                else {
                    Bitmap btm2;
    ...
    protected Dialog onCreateDialog(int id)
    {
        ...
        {
            return new TimePickerDialog(AddTheatre.this,TimePickerListener,hour,min,true);
        }

        else if(id == DIALOG_ID_DATE)
        {
            return new DatePickerDialog(AddTheatre.this,DatePickerListener,year,month,day);
        }
        return null;
    }
    ...
    private void selectImageOption()
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(AddTheatre.this);
