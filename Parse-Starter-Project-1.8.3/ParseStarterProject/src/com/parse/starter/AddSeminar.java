...
public class AddSeminar extends ActionBarActivity implements View.OnClickListener
{
    TextView time,date;
    Button camera,done;
    ImageButton map;
    ImageView pic;
    EditText event_name,speaker,address,ticket,description,quota,video;
    TextView city;
    Spinner sCity;
    ...
    String s_event_name,s_speaker,s_address,s_ticket,s_description,s_video;
    int s_quota = -1;
    ...
    public void onClick(View v)
    {
        ...
        if(v == done)
        {
            ...
            s_speaker = speaker.getText().toString();

            boolean check3 = false;
            if ( speaker.getText().toString().equals("") || s_event_name.equals("") || s_address.equals("") || s_quota == -1 ||
                    !isTimeSet || !isDateSet ) {
           ...
           if( ( !Patterns.WEB_URL.matcher(s_video).matches() || !URLUtil.isValidUrl(s_video) ) && !s_video.equals("") ) {
            ...
            if( check1 && check2 && check3) {
                ...
                ParseObject seminar = new ParseObject("Events");
                seminar.put( "rating", 0);
                seminar.put("name", s_event_name);
                seminar.put("organizerID", ParseUser.getCurrentUser().getObjectId());
                seminar.put("comments", new ArrayList<String>());
                seminar.put("date", year * 10000 + month * 100 + day);
                seminar.put("address", s_address);
                seminar.put("subscribers", new ArrayList<String>());
                seminar.put("key", (int) (Math.random() * 1000000));
                seminar.put("category", "Seminar");
                seminar.put("comments", new ArrayList<String>());
                seminar.put("city", sCity.getSelectedItem());
                seminar.put("isMapClicked", flag2);
                seminar.put("latitude", lat);
                seminar.put("longitude", lon);

                seminar.put("rating", 0 );
                seminar.put("numComment", 0);

                Bitmap btm;
                if (eventImage == null) {
                    btm = BitmapFactory.decodeResource(getResources(), R.drawable.seminar);
                } else {
                ...
                seminar.put("eventImage", file);

                seminar.put("videoLink", s_video);
                seminar.put("ticketLink", s_ticket);
                seminar.put("description", s_description);
                seminar.put("quota", s_quota);
                seminar.put("speaker", s_speaker );
                seminar.put("time", hour * 100 + min);

                seminar.put("organizerName", ParseUser.getCurrentUser().getString("username"));
                ...
                if( ParseUser.getCurrentUser().getParseFile("profilePhoto") != null ) {
                    ...
                    seminar.put("organizerPhoto", file2);
                }
                else {
                    Bitmap btm2;
    ...
    protected Dialog onCreateDialog(int id)
    {
        ...
        {
            return new TimePickerDialog(AddSeminar.this,TimePickerListener,hour,min,true);
        }

        else if(id == DIALOG_ID_DATE)
        {
            return new DatePickerDialog(AddSeminar.this,DatePickerListener,year,month,day);
        }
        return null;
    }
    ...
    private void selectImageOption()
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(AddSeminar.this);
