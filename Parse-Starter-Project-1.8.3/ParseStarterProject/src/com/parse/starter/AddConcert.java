...
public class AddConcert extends ActionBarActivity implements View.OnClickListener
{
    TextView time,date;
    Button camera,done;
    ImageButton map;
    ImageView pic;
    EditText event_name,artist,address,ticket,description,quota,video;
    TextView city;
    Spinner sCity;
    ...
    String s_event_name,s_artist,s_address,s_ticket,s_description,s_video;
    int s_quota = -1;
    ...
    public void onClick(View v)
    {
        ...
        if (v == done) {
            ...
            s_artist = artist.getText().toString();
            ...
            if( s_event_name.equals("") || s_artist.equals("")||s_address.equals("") || s_quota == -1 || !isTimeSet || !isDateSet){
            ...
            if( check1 && check2 && check3) {
                ...
                ParseObject concert = new ParseObject("Events");

                concert.put("name", s_event_name);
                concert.put( "rating", 0);
                concert.put("organizerID", ParseUser.getCurrentUser().getObjectId());
                concert.put("comments", new ArrayList<String>());
                concert.put("date", year * 10000 + month * 100 + day);
                concert.put("address", s_address);
                concert.put("city", sCity.getSelectedItem());
                concert.put("subscribers", new ArrayList<String>());
                concert.put("key", (int) (Math.random() * 1000000));
                concert.put("artist", s_artist);
                concert.put("category", "Concert");
                concert.put("comments", new ArrayList<String>());
                concert.put("isMapClicked", flag2);
                concert.put("latitude", lat);
                concert.put("longitude", lon);

                concert.put("rating", 0 );
                concert.put("numComment", 0);

                Bitmap btm;
                if (eventImage == null) {
                    btm = BitmapFactory.decodeResource(getResources(), R.drawable.concert);
                } else {
                ...
                concert.put("eventImage", file);

                concert.put("time", hour * 100 + min);
                concert.put("videoLink", s_video);
                concert.put("ticketLink", s_ticket);
                concert.put("description", s_description);
                concert.put("quota", s_quota);
                concert.put("organizerName", ParseUser.getCurrentUser().getString("username"));
                ...
                if( ParseUser.getCurrentUser().getParseFile("profilePhoto") != null ) {
                    ...
                    concert.put("organizerPhoto", file2);
                }
                else {
                    Bitmap btm2;
    ...
    protected Dialog onCreateDialog(int id)
    {
        ...
        {
            return new TimePickerDialog(AddConcert.this, TimePickerListener, hour,min,true);
        }

        else if(id == DIALOG_ID_DATE)
        {
            return new DatePickerDialog(AddConcert.this,DatePickerListener,year,month,day);
        }
        return null;
    }
    ...
    private void selectImageOption()
    {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(AddConcert.this);
